...
export class Commands {
  ...
  public async download(args: string[]) {
    const dataDdo = await this.ocean.assets.resolve(args[0])
    if (!dataDdo) {
      console.error('Error fetching DDO ' + args[0] + '.  Does this asset exists?')
      return
    }
    const accessService = await this.ocean.assets.getServiceByType(args[0], 'access')
    if (!accessService) {
      console.error(
    ...
    if (!txid) {
      console.error(
        'Error ordering access for ' + args[0] + '.  Do you have enought tokens?'
      )
      return
    }
  ...
  public async compute(args: string[]) {
    ...
    const algoDdo = await this.ocean.assets.resolve(args[1])
    if (!algoDdo) {
      console.error('Error resolving ' + args[0] + '.  Does this asset exists?')
      return
    }
    const computeService = await this.ocean.assets.getServiceByType(args[0], 'compute')
    if (!computeService) {
      console.error(
    ...
    const algoService = await this.ocean.assets.getServiceByType(args[1], 'access')
    if (!algoService) {
      console.error(
    ...
    if (!computeOrderId) {
      console.error(
        'Error ordering compute for ' + args[0] + '.  Do you have enought tokens?'
      )
      return
    }
  ...
  public async allowAlgo(args: string[]) {
    ...
    const computePrivacy = computeService.attributes.main.privacy
  ...
  public async disallowAlgo(args: string[]) {
    ...
    const computePrivacy = computeService.attributes.main.privacy
    if (computePrivacy.trustedAlgorithms.includes(args[1]))
      computePrivacy.trustedAlgorithms = computePrivacy.trustedAlgorithms.filter(
        (item) => item !== args[1]
      )
    const newDdo = await this.ocean.assets.updateComputePrivacy(
      ddo.id,
