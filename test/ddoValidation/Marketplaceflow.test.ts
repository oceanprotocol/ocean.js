...
describe('Marketplace flow', () => {
  ...
  it('Generates metadata without index field', async () => {
    asset = {
      main: {
        type: 'dataset',
        name: 'test-dataset',
  ...
  it('Alice FAILS to publish a dataset with metadata without index field', async () => {
  ...
  it('Generates metadata with index', async () => {
    asset = {
      main: {
        type: 'dataset',
        name: 'test-dataset',
  ...
  it('Alice publishes a dataset with index field in metadata', async () => {
    price = '10' // in datatoken
    const publishedDate = new Date(Date.now()).toISOString().split('.')[0] + 'Z'
    const timeout = 0
    service1 = await ocean.assets.createAccessServiceAttributes(
  ...
  xit('Marketplace posts asset for sale', async () => {
  ...
  xit('Bob consumes asset 1', async () => {
  ...
  xit('Bob consumes same asset again, without paying', async () => {
  ...
  xit('owner can list their assets', async () => {
  ...
  xit('Alice updates timeout for the access service', async () => {
  ...
  xit('Alice should check if her asset has valid url(s)', async () => {
  ...
  xit('Alice publishes a dataset but passed data token is invalid', async () => {
    price = '10' // in datatoken
    const publishedDate = new Date(Date.now()).toISOString().split('.')[0] + 'Z'
    const timeout = 0
    service1 = await ocean.assets.createAccessServiceAttributes(
  ...
  xit('Alice publishes a dataset but created data token is invalid', async () => {
    price = '10' // in datatoken
    const publishedDate = new Date(Date.now()).toISOString().split('.')[0] + 'Z'
    const timeout = 0
    service1 = await ocean.assets.createAccessServiceAttributes(
  ...
  xit('Bob should get his order History', async () => {
    const history = await ocean.assets.getOrderHistory(bob)
    assert(history.length > 0)
  })
  xit('Alice should not get any order History', async () => {
