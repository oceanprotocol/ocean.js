...
describe('SideStaking unit test', () => {
  ...
  describe('Test a pool with DAI (18 Decimals)', () => {
    ...
    it('#getDatatokenCirculatingSupply - should get datatoken supply in circulation (vesting amount excluded)', async () => {
      expect(await sideStaking.getDatatokenCirculatingSupply(datatoken)).to.equal(
    ...
    it('#getDatatokenCurrentCirculatingSupply - should get datatoken supply in circulation ', async () => {
      expect(await sideStaking.getDatatokenCurrentCirculatingSupply(datatoken)).to.equal(
        web3.utils.toWei(BASE_TOKEN_LIQUIDITY.toString())
      )
    })

    it('#getBaseToken - should get baseToken address', async () => {
      expect(await sideStaking.getBaseToken(datatoken)).to.equal(contracts.daiAddress)
    })

    it('#getPoolAddress - should get pool address', async () => {
      expect(await sideStaking.getPoolAddress(datatoken)).to.equal(poolAddress)
    })

    it('#getPublisherAddress - should get publisher address', async () => {
      expect(await sideStaking.getPublisherAddress(datatoken)).to.equal(factoryOwner)
    })

    it('#getBaseTokenBalance ', async () => {
      expect(await sideStaking.getBaseTokenBalance(datatoken)).to.equal('0')
    })

    it('#getDatatokenBalance ', async () => {
      expect(
        await (await sideStaking.getDatatokenBalance(datatoken)).toString()
      ).to.equal(
        new BigNumber(2)
          .exponentiatedBy(256)
          .minus(1)
    ...
    it('#getvestingAmount ', async () => {
      expect(await sideStaking.getvestingAmount(datatoken)).to.equal('0')
    })

    it('#getvestingLastBlock ', async () => {
      expect(await sideStaking.getvestingLastBlock(datatoken)).to.equal(
        initialBlock.toString()
      )
    })

    it('#getvestingAmountSoFar ', async () => {
      expect(await sideStaking.getvestingAmountSoFar(datatoken)).to.equal('0')
    })
