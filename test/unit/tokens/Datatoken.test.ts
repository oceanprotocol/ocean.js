...
describe('Datatoken', () => {
  ...
  let user2: string
  let user3: string
  let datatokenDeployerUser: string
  ...
  before(async () => {
    ...
    user2 = accounts[2]
    user3 = accounts[3]
    datatokenDeployerUser = accounts[4]
  ...
  it('#createDatatoken - should create a new Datatoken from NFT contract', async () => {
    await nftDatatoken.addDatatokenDeployer(nftAddress, nftOwner, datatokenDeployerUser)
  ...
  it('#setPaymentCollector - should set a new paymentCollector, if ERC 20 DEPLOYER', async () => {
    assert(
      (await nftDatatoken.getNftPermissions(nftAddress, datatokenDeployerUser))
        .deployDatatoken === true
    )

    await datatoken.setPaymentCollector(datatokenAddress, datatokenDeployerUser, user3)
